 ...
 * 
 *  조인 테이블 삭제 : 위 순서에 역순이다. (transaction table > master table)
 *  강제 테이블 삭제 : drop table 테이블명 cascade constraint; (transaction table은 유지하고 master table만 삭제하고 싶을때)
 */

-- 1단계 : 기본키가 포함된 테이블 
drop table dept_tab cascade constraint; --  join관계가 존재할때 강제로 table 삭제 
...
insert into dept_tab values(3, '연구소', '대전');
...
insert into emp_tab values(1003, '유관순', 1500000, 3); -- error-- 참조무결성 제약조건 위배 

commit work; -- [db 반영 -> 추가]


-- 단계5 : join으로 데이터 조회
SELECT E.EMPNO, E.ENAME, D.NAME, D.LOC
FROM DEPT_TAB D, EMP_TAB E
WHERE D.DEPTNO = E.DEPTNO AND D.LOC LIKE '대%';

-- 테이르 별칭 : SELECT, WHERE 절 사용
