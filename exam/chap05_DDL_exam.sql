...
-- SQL문
SELECT  AIN.AID, AOUT.NAME
FROM ANIMAL_INS AIN, ANIMAL_OUTS AOUT
WHERE AIN.AID = AOUT.AID AND AIN.DATETIME > AOUT.DATETIME
ORDER BY AIN.DATETIME;

SELECT AIN.AID, AOUT.NAEM 
FROM ANIMAL_INS AIN, AINMAL_OUTS AOUT
WHERE AIN.AID  IN (1001, 1002)
AINKMMIMNJJSL DIKIKJNNJJJ
S

/*
 * [문3] SUBQUERY 이용 
 * 천재지변으로 인해 일부 데이터가 유실되었다. 입양을 간 기록은 있는데,
 * 보호소에 들어온 기록이 없는 동물의 ID와 이름을 ID순으로 조회하는 
 * SQL문을 작성하시오.
 * 사용 테이블 : ANIMAL_INS2, ANIMAL_OUTS
...
SELECT AID
FROM ANIMAL_INS 
WHERE NAME NOT IN (SELECT NAME  FROM ANIMAL_OUTS) AND ROWNUM <= 3 
ORDER BY DATETIME
